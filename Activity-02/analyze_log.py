...
COMMAND_INPUT_PATTERN = re.compile(
    r"\[HoneyPotSSHTransport,\d+,\d+\] Command found: (.+)"
)
...
def analyze_failed_logins(path: str, min_count: int):
    ...
    _print_counter(filtered_ips, "Source IP", "Failed Attempts")
    ...
    with open(path, "r", encoding="utf-8") as file:
        for line in file:
            ...
            if match:
                ...
                # Fix byte-string-like appearance (if any)
                user = str(user).strip("b'\"")
                pw = str(pw).strip("b'\"")

                cred_ip_map[(user, pw)].add(ip)

    sorted_creds = sorted(cred_ip_map.items(), key=lambda item: len(item[1]), reverse=True)
...
def main():
    ...
    if __name__ == "__main__":
        ...
        help="Which analysis to run")
        args.task == "top-commands": # type: ignore
...
# ── CLI ─────────────────────────────────────────────────────────────────────
def analyze_top_commands(path: str, top_n: int = 10):
    """Analyze and print the most common shell commands entered by attackers."""
    command_counter = Counter()

    with open(path, "r", encoding="utf-8") as file:
        for line in file:
            match = COMMAND_INPUT_PATTERN.search(line)
            if match:
                command = match.group(1).strip()
                command_counter[command] += 1

    print(f"Top {top_n} Shell Commands")
